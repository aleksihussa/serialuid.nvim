...
local function get_fqcn_and_classpath(filepath)
  ...
  local root = table.concat(vim.list_slice(parts, 1, java_index), "/")
  ...
  local fqcn = table.concat(class_parts, ".")

  local candidate_classpaths = {
    root .. "/../classes/java/main",
...
function M.generate()
  ...
  local fqcn, classpath = get_fqcn_and_classpath(bufname)
  if fqcn and classpath then
    -- Project-aware serialver execution
    local serialver_cmd = "serialver -classpath " .. classpath .. " " .. fqcn
    ...
    -- Fallback: Compile current file manually
    ...
    local serialver_cmd = "serialver -classpath " .. tmp_dir .. " " .. fallback_fqcn
    local output, serr = run_command(serialver_cmd)
    if serr then
